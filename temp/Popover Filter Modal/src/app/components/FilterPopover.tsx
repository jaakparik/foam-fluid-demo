...
export function FilterPopover() {
  const [topLevelTab, setTopLevelTab] = useState<TopLevelTab>('Creator');
  const [creatorTab, setCreatorTab] = useState<CreatorTab>('Gender');
  const [audienceTab, setAudienceTab] = useState<AudienceTab>('Gender');
  const [selectedPlatforms, setSelectedPlatforms] = useState<PlatformTab[]>([]);
  const [hoveredPlatform, setHoveredPlatform] = useState<PlatformTab | null>(null);
  
  // Filter state tracking
  const [creatorGenderSelection, setCreatorGenderSelection] = useState<string[]>([]);
  const [creatorAgeSelection, setCreatorAgeSelection] = useState({ min: 12, max: 80 });
  const [creatorLocationSelection, setCreatorLocationSelection] = useState<string[]>([]);
  const [creatorVerticalsSelection, setCreatorVerticalsSelection] = useState<string[]>([]);
  
  const [audienceGenderSelection, setAudienceGenderSelection] = useState<{ gender: 'male' | 'female' | null; percentage: number }>({ gender: null, percentage: 0 });
  const [audienceAgeSelection, setAudienceAgeSelection] = useState<{ ages: string[]; range: { min: number; max: number } }>({ ages: [], range: { min: 10, max: 65 } });
  const [audienceLocationSelection, setAudienceLocationSelection] = useState<string | null>(null);
  
  const [platformConfigurations, setPlatformConfigurations] = useState<{[key: string]: any}>({});
