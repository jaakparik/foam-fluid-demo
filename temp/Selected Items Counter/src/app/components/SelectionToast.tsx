...
import svgPaths from "@/imports/svg-xgiv963bzb";
...
function ItemsSelected({ count }: { count: number }) {
  return (
    <div className="content-stretch flex flex-col gap-[2px] h-full items-center justify-center px-[16px] py-0 relative shrink-0" data-name="Items Selected">
      ...
      <p className="css-ew64yg font-['Founders_Grotesk:Medium',sans-serif] leading-[16px] not-italic relative shrink-0 text-[#f9fafa] text-[16px]">{count}</p>
...
function Actions({ onSelectAll, onShare, onAddTo }: { onSelectAll?: () => void; onShare?: () => void; onAddTo?: () => void }) {
  return (
...
export default function SelectionToast({ 
  ...
  return (
    <div className="bg-[#1c2128] relative rounded-[12px] w-fit" data-name="action toast/ Default">
      <div className="content-stretch flex items-center overflow-clip relative rounded-[inherit] h-full">
        ...
        <Actions onSelectAll={onSelectAll} onShare={onShare} onAddTo={onAddTo} />
        <div className="flex flex-row items-center self-stretch">
