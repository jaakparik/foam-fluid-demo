...
export function QuickResults({
  ...
  useEffect(() => {
    if (hasSearchQuery || hasAtMention) {
      ...
      // MEDIAKITS: Show results and count
      timers.push(setTimeout(() => {
        setSectionStates(prev => ({ 
          ...prev, 
  ...
  }, [searchQuery, hasSearchQuery, hasAtMention, selectedMentions]);
    ...
    >
      ...
      {searchQuery && (
        <div className="flex gap-[8px] p-[12px]">
          {(hasAtMention ? talentFilterButtons : filterButtons).map((button, index) => {
