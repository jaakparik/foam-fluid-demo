import svgPaths from "./svg-jbicj23zgf";

function Group() {
  return (
    <div className="absolute inset-[15.63%_14.96%_18.08%_18.75%]">
      <div className="absolute inset-[0_0_-4.53%_-4.53%]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 13.8583 13.8583">
          <g id="Group 7">
            <path d={svgPaths.p14b101a0} id="Vector 631" stroke="var(--stroke-0, #54657D)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" />
            <path d={svgPaths.p3f3aae00} id="Rectangle 556" stroke="var(--stroke-0, #54657D)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" />
          </g>
        </svg>
      </div>
    </div>
  );
}

export default function Pin() {
  return (
    <div className="relative size-full" data-name="Pin">
      <Group />
    </div>
  );
}